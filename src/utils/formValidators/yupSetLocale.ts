...
Yup.setLocale({
  ...
  string: {
    ...
    length(params) {
        return`Deve ter ${params.length} ${params.length === 1 ? 'caractere' : 'caracteres'}`
    },
    min(params) {
        return`Deve ter pelo menos ${params.min} ${params.min === 1 ? 'caractere' : 'caracteres'}`
    },
    max(params) {
        return`Deve ter no máximo ${params.max} ${params.max === 1 ? 'caractere' : 'caracteres'}`
    },
